...
- [x] **F2.AUTH-02** `login`/`logout` via serviço (mock); montar sessão; tratar erro; alternar login/app. → `src/state/SessaoContext.tsx` + `App.tsx`. **Alterado em 2026-08-18** *(fora do escopo original — ver `Claude/eventos-roadmap.md`)*: `Sessao` ganhou `precisaTrocarSenha`; com a flag `true` (login recém-criado ou senha resetada pelo Admin), `App.tsx` mostra `TrocarSenhaObrigatoria.tsx` no lugar do Shell — só a troca de senha é permitida, sem acesso ao resto do sistema. **Ajustado no mesmo dia, mesma sessão, quando `Claude/API (10).md` trouxe `PUT /api/trocar-senha`** (autoatendimento de verdade, qualquer papel, exige a senha atual): a tela ganhou um 3º campo (Senha atual) e a chamada passou desse endpoint novo — funciona pra qualquer colaborador logado agora, não só Admin (a pendência anterior, quando só `PUT /api/usuarios/{id}` só-Admin existia, foi resolvida pela própria API antes de chegar a afetar produção).
...
- [x] **F2.CAD-03** Render da tabela + estados. **Alterado em 2026-08-18** *(fora do escopo original — ver `Claude/eventos-roadmap.md`)*: barra de busca acima da tabela (nome, código, CPF, e-mail ou usuário de acesso — sem diferenciar maiúscula/minúscula nem acento), filtro só no cliente sobre a lista já carregada; visível pra qualquer perfil, não só Admin. Mensagem de vazio muda pra "Nenhum colaborador encontrado para '...'" quando a busca não acha ninguém.
- [x] **F2.CAD-04** Formulário: campos (código, nome, CPF c/ máscara, **filial**, cargo, **perfil**, e-mail, usuário/senha de acesso), **5 checkboxes de habilitação de tela**, validação (exige código, nome, CPF, filial), salvar/editar/cancelar/remover. **Alterado em 2026-08-10:** campo de Filial adicionado ao formulário (era implícito pelo cabeçalho); depois o formulário inline foi substituído por uma modal ("+ Adicionar colaborador"). **Alterado em 2026-08-11:** campo de Perfil adicionado (Vendedor/Coordenador/Gerente/Administrador). **Alterado em 2026-08-18** *(fora do escopo original — ver `Claude/eventos-roadmap.md`)*: os botões "Editar"/"Remover" da tabela viraram um ícone "⋮" (`MenuAcoes`, novo componente em `src/components/ui/`) que abre um menu com quatro ações — Editar, **Resetar senha** (grava a senha do colaborador como o próprio CPF via `PUT /api/usuarios/{id}` — enviar `"senha"` já força `"precisa trocar senha": true` na API, exigindo troca no próximo acesso), **Ativar/Inativar colaborador** (alterna `desligado` via o mesmo `PUT /api/usuarios/{id}`; inativar revoga o acesso do colaborador na hora e bloqueia login futuro — `Claude/API (8).md`/`(9).md`; editar outros campos de um colaborador inativado preserva o status, não reativa por engano) e Remover. Nova coluna **Status** na tabela (selo Ativo/Inativo). **Alterado em 2026-08-19** *(fora do escopo original — ver `Claude/eventos-roadmap.md`)*: 5ª opção de Perfil, **Padrão** — sem nenhum acesso ao sistema (`NAV_POR_PAPEL.padrao = []`, guarda de rota do Shell não libera nenhuma tela), usada só para colaboradores que precisam existir no cadastro (e nas demais tabelas que listam colaboradores) sem fazer login.
...
- [x] **F4.COM-02** Serviço + mock: `listarComissoes(filial, mes)`, `salvarComissao(...)`; obter PEV da Premiação para leitura. → já existia de F1; ganhou suporte a `FILIAL_TODAS` (ver eventos-roadmap). **F8.COM iniciado adiantado em 2026-08-12:** `comissaoServiceHttp` (`src/adapters/http/comissaoService.http.ts`) consome `GET/PUT /api/comissoes` fora dos testes — o `pev` já vem calculado ao vivo pela própria API; ver `Claude/eventos-roadmap.md`. **Alterado em 2026-08-21:** `GET /api/comissoes` passou a devolver `filial` em cada linha (`Claude/API (17).md`) — a chamada extra a `/api/usuarios` que só existia pra preencher esse campo foi eliminada.
...
- [x] **F4.DESC-02** Serviço + mock: `listarDescontos(...)`, `salvarDescontos(...)`, `removerDesconto(...)` (persistência por linha). → já existia de F1, sem alterações. **F8.DESC iniciado adiantado em 2026-08-12:** `descontosServiceHttp` (`src/adapters/http/descontosService.http.ts`) consome `GET/PUT/DELETE /api/descontos-bonificacoes` fora dos testes — a API só cria lançamentos (nunca atualiza por id), então editar um lançamento existente apaga e recria; ver `Claude/eventos-roadmap.md`. **Alterado em 2026-08-21:** `listarDescontos` passou a devolver `{colaboradores, lancamentos}` — `GET /api/descontos-bonificacoes` já traz o roster inteiro do mês, com `codigo` (`Claude/API (18).md`), eliminando a chamada extra a `/api/usuarios` que a tela fazia só pra montar a lista de quem pode lançar.
...
- [x] **F5.PS-CAD-06** Checkboxes de **adesão Saúde/Odonto** por titular (default verdadeiro; **só Admin edita**; demais `disabled`). **Alterado em 2026-08-17:** dependentes também ganharam checkboxes próprias e marcáveis (antes eram só um ✓/— espelhando a adesão do titular) — adesão independente por pessoa, controla quem aparece na grade de Lançamento. **Alterado em 2026-08-18:** a API ganhou `"plano saude"`/`"plano odontologico"` em `PUT /api/dependentes/{id}` (`Claude/API (8).md`) — a adesão do dependente passou a persistir de verdade no HTTP real também (antes só no mock); cada checkbox (titular e dependente) ganhou um indicador visual de "Salvando…" enquanto a alteração está em voo. Ver `Claude/eventos-roadmap.md`.
...
- [x] **F5.PS-LAN-02** Serviço + mock: `listarLancamentosPlanoSaude(filial, mes, tipo)`, `salvarLancamentoPlanoSaude(...)`. **Alterado em 2026-08-21:** `listarLancamentosPlanoSaude` passou a devolver `{pessoas, totalDesligados}` — `GET /api/lancamentos` já traz o roster inteiro pronto (`Claude/API (19).md`), com `codigo`/`cpf`/`nome`/`filial`/valores já calculados do período vigente por pessoa (titular e dependente). Eliminou 3 fontes de chamadas que a tela fazia à parte: colaboradores, `listarDependentes` (uma por titular — N+1) e `listarPeriodosPlanoSaude`.
- [x] **F5.PS-LAN-03** Render **uma linha por pessoa** (titular + dependentes), filtrando famílias sem adesão ao tipo + estados.
- [x] **F5.PS-LAN-04** Descrição (TITULAR/DEPENDENTE, texto fixo — **nunca editável, nem pelo Admin**; ver decisão em `Claude/eventos-roadmap.md`, o protótipo contradiz uma frase do documento técnico); valor (mesmo para Titular e Dependente) vindo do período vigente cadastrado em `F5.PS-PER` (nunca digitado na grade de Lançamento); célula `***` na coluna não aplicável. **Alterado em 2026-08-14:** antes o valor era fixo por filial/tipo (185,27 padrão · 255,54 nas filiais 401/403 · 13,56 odonto); agora vem do período cadastrado pelo Admin — ver `F5.PS-PER` e `Claude/eventos-roadmap.md`. **Correção em 2026-08-18:** o valor de Odontológico podia mostrar um período antigo já encerrado em vez do vigente, por um bug de desempate em `encontrarPeriodoPlano` — corrigido, ver `Claude/eventos-roadmap.md`.
- [x] **F5.PS-LAN-05** Rodapé com três linhas, todas as colunas calculadas — **Total ativos** (soma de tudo na grade, era o único total antes), **Total desligados** e **Total geral** (= ativos + desligados, coluna a coluna). *(F5.PS-LAN-05b, adicionado em 2026-08-18, fora do escopo original — ver `Claude/eventos-roadmap.md`)*: **Total desligados** é agregado por filial + tipo de plano + mês (não itemizado por colaborador desligado), mas com a mesma composição de colunas da grade normal — 4 campos próprios (Titular/Dependente/Adicional/Coopart., os dois últimos só em Saúde), editável nas duas sub-abas, só numa filial específica (a API não aceita gravar com "todas"). **Ajustado no mesmo dia, mesma sessão, quando `Claude/API (10).md` trouxe a mesma composição de colunas pro `GET`/`PUT /api/lancamentos/desligados`** (antes só existia `valor` único): a quebra por coluna agora é gravada e recuperada de verdade — sobrevive a um recarregamento, sem precisar mais do aviso de "campos voltam a zero" que a versão anterior tinha.
